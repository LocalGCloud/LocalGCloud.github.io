...
### Requirement: Runtime MCP documentation gate

LocalCloud SHALL promote its MCP capability only through the runtime repository’s canonical integration guide.

#### Scenario: MCP integration is promoted

- **WHEN** site or community content mentions MCP support
- **THEN** it SHALL link to the runtime-owned guide for the `/mcp` endpoint and `localcloud mcp` stdio bridge
- **AND** SHALL not publish a parallel package, copied tool catalog, or independent protocol claims from this site repository.

#### Scenario: Early users report failures

- **WHEN** launch feedback identifies setup, compatibility, or agent misrouting failures
- **THEN** those failures SHALL be logged and converted into site documentation updates, runtime issues, skill updates, or product backlog items before the next promotion wave.
