...
<CardGroup cols={2}>
  ...
  <Card title="What is a GCP Emulator?" icon="book" href="/docs/what-is-gcp-emulator/">
    Understand emulators and why you need one.
  </Card>
...
Choose either supported host installation channel:

<CodeTabs labels={["Install script", "Homebrew"]}>
  <Fragment slot="tab-0">
    <pre><code>{productFacts.installScriptCommand}</code></pre>
  </Fragment>
  <Fragment slot="tab-1">
    <pre><code>{productFacts.homebrewInstallCommand}</code></pre>
  </Fragment>
</CodeTabs>

The install script supports macOS and Linux, verifies the selected release archive, and can run diagnostics and start LocalCloud after you confirm. Homebrew users run the first start explicitly:
...
`localcloud doctor` diagnoses Docker but never installs or starts Docker Desktop, Colima, or Docker Engine. Release archives, checksums, and Sigstore bundles are published in the <a href={`${productFacts.githubUrl}/releases`}>CLI releases</a>; the formula source is in the <a href={productFacts.homebrewTapUrl}>Homebrew tap</a>.

## Manual Docker path

Use the direct-Docker flow below on unsupported CLI hosts or when you intentionally do not want the host CLI.
...
docker run -d \
  -p 8080:24080 -p 4443:24081 -p 8085-8087:24082-8087 \
...
See the [Terraform Integration](/docs/terraform/) guide for details.
...
```bash
curl http://localhost:24080/_localcloud/health | jq
```

Or visit the web console at [http://localhost:24080](http://localhost:24080).
