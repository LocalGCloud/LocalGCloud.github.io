...
<Callout type="note" title="Check your CLI version">
These instructions cover LocalCloud CLI 0.1.2. Run <code>localcloud --version</code> to confirm the installed version, and use the checksum published with the release you install.
</Callout>
...
configuration's `host.data_volume`, the active-runtime record, then the
...
<CardGroup cols={2}>
  ...
  <Card title="Service Catalog" icon="grid" href="/services/">
    Browse all {publishedServiceCount} available service guides and their documented limitations.
  </Card>
