...
When you run `eval $(curl -s 'http://localhost:24080/_localcloud/env?format=terraform')`, the following variables are set:

| Variable | Value | Service |
|----------|-------|---------|
| `GOOGLE_STORAGE_CUSTOM_ENDPOINT` | `http://localhost:24081` | Cloud Storage |
| `GOOGLE_PUBSUB_CUSTOM_ENDPOINT` | `http://localhost:24082` | Pub/Sub |
| `GOOGLE_BIGQUERY_CUSTOM_ENDPOINT` | `http://localhost:24087` | BigQuery |
| `GOOGLE_SPANNER_CUSTOM_ENDPOINT` | `http://localhost:24086` | Spanner (REST) |
| `GOOGLE_FIRESTORE_CUSTOM_ENDPOINT` | `http://localhost:24083` | Firestore |
| `GOOGLE_BIGTABLE_CUSTOM_ENDPOINT` | `http://localhost:24084` | Bigtable |
| `GOOGLE_SECRET_MANAGER_CUSTOM_ENDPOINT` | `http://localhost:24080` | Secret Manager |
| `GOOGLE_CLOUD_TASKS_CUSTOM_ENDPOINT` | `http://localhost:24080` | Cloud Tasks |
| `GOOGLE_COMPUTE_CUSTOM_ENDPOINT` | `http://localhost:24080` | Compute Engine |
| `GOOGLE_CLOUD_RUN_CUSTOM_ENDPOINT` | `http://localhost:24080` | Cloud Run |
| `GOOGLE_CONTAINER_CUSTOM_ENDPOINT` | `http://localhost:24080` | GKE |
| `GOOGLE_LOGGING_CUSTOM_ENDPOINT` | `http://localhost:24080` | Cloud Logging |
| `GOOGLE_MONITORING_CUSTOM_ENDPOINT` | `http://localhost:24080` | Cloud Monitoring |
  ...
  terraform-test:
    ...
    steps:
      ...
      - name: Configure Terraform for LocalCloud
        run: eval $(curl -s http://localhost:24080/_localcloud/env?format=terraform) >> $GITHUB_ENV
...
For agent-assisted Terraform validation, use the [LocalCloud Agent Skills](https://github.com/LocalGCloud/LocalGCloud.github.io/tree/main/agent-skills). The `localcloud-terraform` skill keeps the standard `hashicorp/google` provider, uses LocalCloud endpoint overrides, and avoids real GCP credentials for local checks.
