...
|----------|-------|---------|
| `STORAGE_EMULATOR_HOST` | `http://localhost:24081` | Cloud Storage |
| `PUBSUB_EMULATOR_HOST` | `localhost:24082` | Pub/Sub |
| `FIRESTORE_EMULATOR_HOST` | `localhost:24083` | Firestore |
| `BIGTABLE_EMULATOR_HOST` | `localhost:24084` | Bigtable |
| `SPANNER_EMULATOR_HOST` | `localhost:24085` | Spanner |
| `BIGQUERY_EMULATOR_HOST` | `http://localhost:24087` | BigQuery |
| `SECRET_MANAGER_EMULATOR_HOST` | `localhost:24080` | Secret Manager |
| `CLOUD_TASKS_EMULATOR_HOST` | `localhost:24080` | Cloud Tasks |
...
For Terraform, use the `terraform` format to export `GOOGLE_*_CUSTOM_ENDPOINT` variables:

```bash
eval $(curl -s 'http://localhost:24080/_localcloud/env?format=terraform')
```

This sets endpoints like `GOOGLE_STORAGE_CUSTOM_ENDPOINT`, `GOOGLE_PUBSUB_CUSTOM_ENDPOINT`, etc. and bypasses authentication with `GOOGLE_APPLICATION_CREDENTIALS=/dev/null`. See the [Terraform Integration](/docs/terraform/) guide for details.
